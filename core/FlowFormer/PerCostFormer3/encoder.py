...
from ..encoders import convnext_large, twins_svt_large
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x, mask_for_patch1=None, mask_for_patch2=None, mask_for_patch3=None):
        ...
        patch_coord = (
            coords_grid(B, out_size[0], out_size[1]).to(x.device) * self.patch_size + self.patch_size / 2
...
class SelfAttentionLayer(nn.Module):
    def __init__(self, dim, cfg, num_heads=8, attn_drop=0.0, proj_drop=0.0, drop_path=0.0, dropout=0.0):
        ...
        self.ffn = nn.Sequential(
            nn.Linear(dim, dim), nn.GELU(), nn.Dropout(dropout), nn.Linear(dim, dim), nn.Dropout(dropout)
        )
...
class CostPerceiverEncoder(nn.Module):
    ...
    def forward(self, cost_volume, data, context=None):
        ...
        cost_maps = (
            cost_volume.permute(0, 2, 3, 1, 4, 5).contiguous().view(B * H1 * W1, self.cfg.cost_heads_num, H2, W2)
        )
    ...
    def pretrain_forward(self, cost_volume_outter, cost_volume, data, context=None, mask=None):
        ...
        ids_keep, mask_for_keys, mask_for_patch1, mask_for_patch2, mask_for_patch3, ids_restore = self.random_masking(
            cost_maps, self.cfg.mask_ratio, mask
        )
    ...
    def __init__(self, cfg):
        ...
        if cfg.fnet == "twins":
            ...
            self.feat_encoder = twins_svt_large_jihao(
                pretrained=self.cfg.pretrain, del_layers=cfg.del_layers, version=cfg.fnet
            )
